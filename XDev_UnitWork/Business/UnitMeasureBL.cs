...
{
    public class UnitMeasureBL : GenericBL<IUnitMeasureRep>, IUnitMeasureBL
    {
        public UnitMeasureBL(ApplicationDbContext dbContext, IHttpContextAccessor contextAccessor, IMapper mapper) : base(dbContext, contextAccessor, mapper)
        {
        }
        ...
        public async Task CreateAsync(UnitMeasureDTO dto)
        {
            var model = await Repository.GetFirstorDefaultAsync(f => f.Code == dto.Code);
            if (model is null)
            {
                model = Mapper.Map<UnitMeasure>(dto);
        ...
        public async Task<UnitMeasureDTO> GetByIdAsync(params object[] ids)
        {
            var model = await Repository.GetByIdAsync(ids);
            if (model is null)
                return new UnitMeasureDTO();

            return Mapper.Map<UnitMeasureDTO>(model);
        }

        public async Task<List<UnitMeasureDTO>> GetListAsync(PaginationDTO pagination)
        {
            var query = await Repository.QueryAsync();
            query = query.CreateFilterAndOrder(pagination);
            return await query.CreatePaging<UnitMeasure, UnitMeasureDTO>(pagination, ContextAccessor.HttpContext);
        }

        public async Task<List<UnitMeasureDTO>> GetListAsync()
        {
            var list = await Repository.GetListAsync();
            return Mapper.Map<List<UnitMeasureDTO>>(list.Select(s => new UnitMeasureDTO
            {
                Id = s.Id,
                Name = s.Name,
                Code = s.Code,
        ...
        public async Task UpdateAsync(UnitMeasureDTO dto)
        {
