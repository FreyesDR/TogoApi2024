...
namespace XDev_UnitWork.Business
{
    public class FeSvBL : IFeSvBL
    {
        ...
        private async Task<FeResponseDTO> SendFacturaAsync(Invoice invoice)
        {
            ...
            var genNum = dbContext.Database.SqlQuery<long>($"select xsp_ebilling_co_next_number({ebillingCom.CompanyId}::uuid,{invoice.InvoiceTypeId}::uuid)").ToList();
            if (genNum.Count == 0)
                throw new CustomTogoException("Error generando rango de número");
            ...
            if (result.StatusCode.Substring(0, 2) == "20")
            {
                ...
                await dbContext.Database.ExecuteSqlAsync($"UPDATE \"SaleOrder\" SET \"Invoiced\" = true WHERE \"Id\"={invoice.SaleOrderId}::uuid");
            }
