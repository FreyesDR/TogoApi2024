...
{
    public class IDTypeBL : GenericBL<IIDTypeRep>, IIDTypeBL
    {
        public IDTypeBL(ApplicationDbContext dbContext, IHttpContextAccessor contextAccessor, IMapper mapper) : base(dbContext, contextAccessor, mapper)
        {
        }
        ...
        public async Task CreateAsync(IDTypeDTO dto)
        {
            var model = await Repository.GetFirstorDefaultAsync(f => f.Code == dto.Code);
            if (model is null)
            {
                model = Mapper.Map<IDType>(dto);
        ...
        public async Task<IDTypeDTO> GetByIdAsync(params object[] ids)
        {
            var model = await Repository.GetByIdAsync(ids);
            if (model is null)
                return new IDTypeDTO();

            return Mapper.Map<IDTypeDTO>(model);
        }

        public async Task<List<IDTypeDTO>> GetListAsync(PaginationDTO pagination)
        {
            var query = await Repository.QueryAsync();
            query = query.CreateFilterAndOrder(pagination);
            return await query.CreatePaging<IDType, IDTypeDTO>(pagination, ContextAccessor.HttpContext);
        }

        public async Task<List<IDTypeDTO>> GetListAsync()
        {
            var list = await Repository.GetListAsync();
            return Mapper.Map<List<IDTypeDTO>>(list.Select(s => new IDTypeDTO
            {
                Id = s.Id,
                Name = s.Name,
                Code = s.Code
        ...
        public async Task UpdateAsync(IDTypeDTO dto)
        {
