...
{
    public class SaleOrderTypeBL : GenericBL<ISaleOrderTypeRep>, ISaleOrderTypeBL
    {
        public SaleOrderTypeBL(ApplicationDbContext dbContext, IHttpContextAccessor contextAccessor, IMapper mapper) : base(dbContext, contextAccessor, mapper)
        {
        }
        ...
        public async Task CreateAsync(SaleOrderTypeDTO dto)
        {
            var model = await Repository.GetFirstorDefaultAsync(f => f.Code == dto.Code);
            if (model is null)
            {
                model = Mapper.Map<SaleOrderType>(dto);
        ...
        public async Task<SaleOrderTypeDTO> GetByIdAsync(params object[] ids)
        {
            var model = await Repository.GetByIdAsync(ids);
            if (model is null)
                return new SaleOrderTypeDTO();

            return Mapper.Map<SaleOrderTypeDTO>(model);
        }

        public async Task<List<SaleOrderTypeDTO>> GetListAsync(PaginationDTO pagination)
        {
            var query = await Repository.QueryAsync();
            query = query.CreateFilterAndOrder(pagination);
            return await query.CreatePaging<SaleOrderType, SaleOrderTypeDTO>(pagination, ContextAccessor.HttpContext);
        }

        public async Task<List<SaleOrderTypeDTO>> GetListAsync()
        {
            var list = await Repository.GetListAsync();
            return Mapper.Map<List<SaleOrderTypeDTO>>(list.Select(s => new SaleOrderTypeDTO
            {
                Id = s.Id,
                Name = s.Name,
                Code = s.Code,
        ...
        public async Task UpdateAsync(SaleOrderTypeDTO dto)
        {
            ...
            try
            {
                ...
                {
                    model.Name = dto.Name;
                    model.Delivery = dto.Delivery;                    
                    model.Invoice = dto.Invoice;
                    model.InvoiceTypeId = dto.InvoiceTypeId;
