...
{
    public class InvoiceTypeBL : GenericBL<IInvoiceTypeRep>, IInvoiceTypeBL
    {
        public InvoiceTypeBL(ApplicationDbContext dbContext, IHttpContextAccessor contextAccessor, IMapper mapper) : base(dbContext, contextAccessor, mapper)
        {
        }
        ...
        public async Task CreateAsync(InvoiceTypeDTO dto)
        {
            var model = await Repository.GetFirstorDefaultAsync(f => f.Code == dto.Code);
            if (model is null)
            {
                model = Mapper.Map<InvoiceType>(dto);
        ...
        public async Task<InvoiceTypeDTO> GetByIdAsync(params object[] ids)
        {
            var model = await Repository.GetByIdAsync(ids);
            if (model is null)
                return new InvoiceTypeDTO();

            return Mapper.Map<InvoiceTypeDTO>(model);
        }

        public async Task<List<InvoiceTypeDTO>> GetListAsync(PaginationDTO pagination)
        {
            var query = await Repository.QueryAsync();
            query = query.CreateFilterAndOrder(pagination);
            return await query.CreatePaging<InvoiceType, InvoiceTypeDTO>(pagination, ContextAccessor.HttpContext);
        }

        public async Task<List<InvoiceTypeDTO>> GetListAsync()
        {
            var list = await Repository.GetListAsync();
            return Mapper.Map<List<InvoiceTypeDTO>>(list.Select(s => new InvoiceTypeDTO
            {
                Id = s.Id,
                Name = s.Name,
                Code = s.Code
        ...
        public async Task UpdateAsync(InvoiceTypeDTO dto)
        {
